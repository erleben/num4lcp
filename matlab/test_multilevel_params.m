...
display( strcat( 'Parameter test: multilevel = ', m1(f1), ' in ', num2str(i1), ' iterations' ) );

[z2 e2 i2 f2 conv2 m2] = multilevel(A, b, x0, k2, max_iter, tol_rel, tol_abs, true);
display( strcat( 'Parameter test: multilevel = ', m2(f2), ' in ', num2str(i2), ' iterations' ) );

[z3 e3 i3 f3 conv3 m3] = multilevel(A, b, x0, k3, max_iter, tol_rel, tol_abs, true);
display( strcat( 'Parameter test: multilevel = ', m3(f3), ' in ', num2str(i3), ' iterations' ) );

[z4 e4 i4 f4 conv4 m4] = multilevel(A, b, x0, k4, max_iter, tol_rel, tol_abs, true);
display( strcat( 'Parameter test: multilevel = ', m4(f4), ' in ', num2str(i4), ' iterations' ) );

[z5 e5 i5 f5 conv5 m5] = multilevel(A, b, x0, k5, max_iter, tol_rel, tol_abs, true);
display( strcat( 'Parameter test: multilevel = ', m5(f5), ' in ', num2str(i5), ' iterations' ) );
